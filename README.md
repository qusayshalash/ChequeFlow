...
> على جهاز حقيقي، بدّل `localhost` في `EXPO_PUBLIC_API_URL` داخل
> `apps/mobile/.env` إلى عنوان IP للشبكة المحلية. لكلٍّ من التطبيقين ملف
> بيئة في مجلده — Next وExpo لا يقرآن `.env` الجذر — والنسختان النموذجيتان
> في `apps/web/.env.example` و`apps/mobile/.env.example`.
>
> أما لوحة الويب فلا تحتاج إعدادًا لتُفتح من الجوال: تستنتج عنوان الـAPI من
> العنوان الذي فُتحت منه، والـAPI في بيئة التطوير يقبل أي عنوان على الشبكة
> المحلية دون إضافته إلى `CORS_ORIGINS`.
